...
# Fleet Animator
Set sail with Fleet Animator, an animation application for creating motion graphics, and soon to be for making frame-by-frame as well as interpolated animation. Fleet supports 2D animation, with an emphasis of reusable objects and easy to use controls. Fleet, built using Godot, gains a fast UI system and cross-platform compatibility. Unlike a traditional animation program or animation using paper, Fleet supports an intuitive graph-based workflow. This design is made to foster creativity and allow expansion into never before seen islands. So let's jump on board and head to the amazing!
